...
def test_npz(tmp_path):
    ...
    np.savez_compressed(path, X_train=rng.random((160, 14)), X_test=X_test, y_train=rng.integers(0, 2, size=160), y_test=y_test, feature_names=np.array([f'f{i}' for i in range(14)]))
    return path
...
class TestSaveLoadMetrics:
    ...
    def test_load_matches_saved(self, tmp_path, synthetic_preds):
        ...
        assert abs(loaded['accuracy'] - m['accuracy']) < 1e-06
