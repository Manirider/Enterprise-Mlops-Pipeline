"""
Enterprise MLOps Pipeline — Test Suite: Pipeline Integration
=============================================================
End-to-end integration tests validating the full DVC pipeline
executes correctly as a sequence of Python function calls
without requiring DVC binary to be installed.
"""
...
def pipeline_dir(tmp_path):
    """Set up a minimal pipeline workspace with params.yaml and adult.csv."""
    import yaml, os, shutil

    # Write params.yaml
    params = {
        "base": {"random_state": 42, "log_level": "INFO", "project_name": "test"},
        "data": {
            "raw_data_path": str(tmp_path / "data" / "adult.csv"),
            "processed_data_path": str(tmp_path / "data" / "processed.csv"),
            "features_path": str(tmp_path / "data" / "features.npz"),
        ...
        "evaluate": {
            "metrics_path": str(tmp_path / "metrics" / "scores.json"),
    ...
    # Generate synthetic adult.csv
    rng = np.random.default_rng(42)
    n = 300
    df = pd.DataFrame({
        "age": rng.integers(18, 90, n),
        "workclass": rng.choice(["Private", "Self-emp-not-inc", "State-gov"], n),
        "fnlwgt": rng.integers(50000, 500000, n),
        "education": rng.choice(["Bachelors", "HS-grad", "Masters"], n),
        "education_num": rng.integers(5, 16, n),
        "marital_status": rng.choice(["Never-married", "Married-civ-spouse", "Divorced"], n),
        "occupation": rng.choice(["Exec-managerial", "Prof-specialty", "Adm-clerical"], n),
    ...
    # Patch os.chdir so relative paths in pipeline code work
...
class TestFullPipelineIntegration:
    """Integration tests running all 4 pipeline stages in sequence."""

    def test_prepare_creates_processed_csv(self, pipeline_dir):
        """Stage 1 must create processed.csv."""
        import importlib, sys

        # Clear lru_cache and re-import config with patched params
    ...
    def test_featurize_creates_npz(self, pipeline_dir):
        """Stages 1-2 must create features.npz."""
        import sys
    ...
    def test_train_creates_model(self, pipeline_dir):
        """Stages 1-3 must create model.joblib."""
        import sys
    ...
    def test_evaluate_creates_metrics(self, pipeline_dir):
        """Full 4-stage pipeline must produce scores.json with expected keys."""
        import sys, json
